...
ReportManager.prototype.writeReports = function () {
    ...
    function addTest(time, className, name, failureMessage) {
        strReport += "<testcase time='" + (time / 1000).toFixed(2) + "' classname='" + className  + "' name='" + name + "'";
        if (failureMessage) {
            if (failureMessage === "skip") {
                strReport += " ' executed='false' />";
            } else {
                strReport += "><failure><![CDATA[" + failureMessage + "]]></failure>" + "</testcase>";
            }
        } else {
            strReport += "/>";
        }
                    ...
                    for (m in testJson[l]) {
                        if (testJson[l][m].type === "test") {
                            ...
                            if (testJson[l][m].result === "fail") {
                                ...
                                addTest(timePerTest, className, testName,"skip");
                            }
                        }
                    }
                }
    ...
    totalCount = totalCount + missingErrorCount;
    failCount = failCount + missingErrorCount;


    log();
    log("Test Report Summary".bold.magenta);
    log();
    log(("Total Number of Executed Tests: " + totalCount ).blue);
    log(("Total Number of Passed Tests: " + passCount ).green);
    log(("Total Number of Failed Tests: " + failCount ).red);
